...
export default async function MovieDetails({ params: { id } }: movieDetailsProps) {
  ...
  return (
    <div>
        ...
        <div className="py-10 flex flex-col lg:flex-row items-center gap-5">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-2">
            ...
            <p className="text-gray-200 text-sm">
              IMDB: <span className="text-white font-medium">{details.vote_average}</span>
            </p>
            <p className="text-gray-200 text-sm">
              Votes: <span className="text-white font-medium">{details.vote_count}</span>
            </p>
            <p className="text-gray-200 text-sm">
              Release Data: <span className="text-white font-medium">{details.release_date}</span>
            </p>
            <p className="text-gray-200 text-sm">
              Genres:{" "}
            ...
            <p className="text-gray-200 text-sm">
              Tag Line: <span className="text-white font-medium">{details.tagline}</span>
            </p>
            <p className="text-gray-200 text-sm">
              Status: <span className={`font-medium ${details?.status === "Released" ? "text-green-500" : "text-red-500"}`}>{details.status}</span>
            </p>
          </div>
        </div>
      ...
      <div className="mt-6">
        <MovieContainer movies={popoularMovies} title="Popular Movies" isVertical />
      </div>
    </div>
  );
}
